...
class SaveConfigFile():
    ...
    def validate_alg_name(self,alg_name):
        if alg_name != '':
            self.save_algorithm_settings(alg_name)
        else:
            return False    
    def save_algorithm_settings(self, alg_name):
        ...
        for alg in self.root.findall("./Solve/Algorithm"):
            if alg.get('name') == alg_name:
                alg.find('Status').text = self.STATUS_A
            else:
                alg.find('Status').text = self.STATUS_I            
