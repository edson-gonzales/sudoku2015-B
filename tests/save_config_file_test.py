...
class SaveConfigFileTest(unittest.TestCase):
    ...
    def setUp(self):
        self.save_config_test = 'saveconfigtest.xml'

    def test_outputfilepath_changed_is_saved_in_config_file(self):

        new_file_path = 'c:\sudokugame\sudoku.txt'
        SaveConfigFile(self.save_config_test).save_output_file_settings(new_file_path)
        get_file_path = ReadConfigFile(self.save_config_test).get_output_file()
    ...
    def test_txt_supported_file_type_is_saved_in_config_file(self):
        ...
        SaveConfigFile(self.save_config_test).save_output_file_settings(txt_file_type)
        get_file_path = ReadConfigFile(self.save_config_test).get_output_file()        
    ...
    def test_html_supported_file_type_is_saved_in_config_file(self):
        ...
        SaveConfigFile(self.save_config_test).save_output_file_settings(html_file_type)
        get_file_path = ReadConfigFile(self.save_config_test).get_output_file()        
    ...
    def test_non_supported_file_type_is_not_saved_in_config_file(self):
        ...
        self.assertFalse(SaveConfigFile(self.save_config_test).save_output_file_settings(other_file_type))
        
    def test_change_BruteForce_algorithm_to_Backtracking_in_config_file(self):

        active_alg = 'Bruteforce'
        SaveConfigFile(self.save_config_test).validate_alg_name(active_alg)
        change_alg_to = 'Backtracking'
        SaveConfigFile(self.save_config_test).validate_alg_name(change_alg_to)
        get_active_alg = ReadConfigFile(self.save_config_test).get_list_of_algorithms()
        self.assertEqual(change_alg_to, get_active_alg[0])

    def test_empty_algoritm_name_is_not_saved_in_config_file(self):

        empty_alg_name = ''
        self.assertFalse(SaveConfigFile(self.save_config_test).validate_alg_name(empty_alg_name))

    def test_change_from_easy_to_medium_level_in_config_file(self):

        active_level = ['Easy', 70, 60]        
        SaveConfigFile(self.save_config_test).save_level_settings(active_level)
        change_level_to = ['Medium', 50, 40]
        SaveConfigFile(self.save_config_test).save_level_settings(change_level_to)
        get_level_name = ReadConfigFile(self.save_config_test).get_list_of_generation_levels_names()
    ...
    def test_change_top_limit_of_custom_level_in_config_file(self):            
        ...
        SaveConfigFile(self.save_config_test).save_level_settings(change_top_limit)
        get_level = ReadConfigFile(self.save_config_test).get_details_of_generation_levels(change_top_limit[0])
    ...
    def test_change_bottom_limit_of_custom_level_in_config_file(self):    
        ...
        SaveConfigFile(self.save_config_test).save_level_settings(change_bottom_limit)
        get_level = ReadConfigFile(self.save_config_test).get_details_of_generation_levels(change_bottom_limit[0])
